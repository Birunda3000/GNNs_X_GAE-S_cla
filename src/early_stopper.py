...
class EarlyStopper:
    ...
    Interface clássica usada pelos classificadores supervisionados:
    ``check(model, epoch, current_value) -> (stop_now, current_value, best_epoch, report)``.
    Mantida lado a lado com o ``UniversalEarlyStopper`` para não quebrar os
    pipelines de classificação e grid search que dependem desta assinatura.
    """
    ...
    def check(
        ...
        Retorna:
            (stop_now, current_value, best_epoch, report)
        """
